variable "num_of_remote_public_agent_group_2" {
  description = "DC/OS Private Agents Count"
  default = 1
}

variable "aws_group_2_remote_public_agent_az" {
...
resource "aws_instance" "remote_public_agent-group-2" {
  ...
  tags {
   ...
   Name =  "${data.template_file.cluster-name.rendered}-remote-pubagt-${var.aws_group_2_remote_public_agent_az}${count.index + 1}"
...
resource "null_resource" "remote_public_agent-group-2" {
  # Changes to any instance of the cluster requires re-provisioning
  triggers {
    cluster_instance_ids = "${null_resource.bootstrap.id}"
    current_ec2_instance_id = "${aws_instance.remote_public_agent-group-2.*.id[count.index]}"
  }
...
#  value = ["${aws_instance.remote_public_agent-group-2.*.public_ip}"]
#}
