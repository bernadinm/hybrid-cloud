variable "num_of_remote_public_agent_group_3" {
  description = "DC/OS Private Agents Count"
  default = 1
}

variable "aws_group_3_remote_public_agent_az" {
  description = "AWS Default Zone"
  default     = "c"
}

resource "aws_instance" "remote_public_agent-group-3" {
  ...
  count = "${var.num_of_remote_public_agent_group_3}"
  ...
  tags {
   ...
   Name =  "${data.template_file.cluster-name.rendered}-remote-pubagt-${var.aws_group_3_remote_public_agent_az}${count.index + 1}"
 ...
 # We run a remote provisioner on the instance after creating it.
  ...
  availability_zone = "${var.aws_remote_region}${var.aws_group_3_remote_public_agent_az}"
}

# Execute generated script on agent
resource "null_resource" "remote_public_agent-group-3" {
  # Changes to any instance of the cluster requires re-provisioning
  triggers {
    cluster_instance_ids = "${null_resource.bootstrap.id}"
    current_ec2_instance_id = "${aws_instance.remote_public_agent-group-3.*.id[count.index]}"
  }

  # Bootstrap script can run on any instance of the cluster
  # So we just choose the first in this case
  connection {
    host = "${element(aws_instance.remote_public_agent-group-3.*.public_ip, count.index)}"
    user = "${module.aws-tested-oses-bursted.user}"
  }
...
#  value = ["${aws_instance.remote_public_agent-group-3.*.public_ip}"]
#}
